...
export const MetaMaskContextProvider = ({ children }: PropsWithChildren) => {
  ...
  const _updateWallet = useCallback(async (providedAccounts?: any[]) => {
    ...
    const isWalletInstance = walletInstanceString
      ? JSON.parse(walletInstanceString)
      : {}; // 기본값으로 빈 객체를 설정
  ...
  const connectKlip = async () => {
    ...
    if (request_key) {
      ...
      const timer: NodeJS.Timer = setInterval(async () => {
        ...
        const { status, address, jwt_token } = await fetch(
          "https://adm-test.sbxg.win/auth/klipResult-test",
          option
        ).then((res) => res.json());
        ...
        if (status === "completed") {
          ...
          localStorage.setItem(
            "walletInstance",
            JSON.stringify({ userAddress: address, logined_platform: "klip" })
          );
