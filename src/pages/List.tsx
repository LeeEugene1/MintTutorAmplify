...
export default function list() {
  ...
  const getNfts = async () => {
    ...
    const res: TutorType[] = await mintAninalTokenContract.methods
      .getAnimalTokens(wallet.accounts[0])
      .call();
    console.log(res);
    const arr: CardType[] = [];
    ...
    try {
      ...
      if (response.status) {
        ...
        const tokenIdx = await mintAninalTokenContract.methods
          .tokenOfOwnerByIndex(wallet.accounts[0], balance.length - 1)
          .call();
  ...
  return (
    <div className="p-8">
      {wallet?.accounts?.length === 0 && (
        <p className="text-xl text-slate-700">Please connect wallet</p>
      )}
      {wallet?.accounts?.length > 0 && (
        <div>
